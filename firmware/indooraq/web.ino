...
#banner { display:none; background:#5a1a1a; color:#ffb3b3; padding:8px 12px; border-radius:8px; margin-bottom:10px; font-size:14px; }
...
<div class="grid">
  <div class="card"><div class="stat">CO2: <span id="co2_val" class="v">--</span> ppm</div><canvas id="co2Chart" height="180"></canvas></div>
...
// How many consecutive failed fetches before we call it "disconnected"
const STALE_AFTER_MISSES = 2;
let consecutiveMisses = 0;

// Same breakpoints as the device's status LED (see leds.cpp / updateLED())
function co2Class(ppm){
    if (ppm < 1000) return "v-ok";
    if (ppm < 1600) return "v-warn";
    return "v-bad";
}
...
async function update(){
    ...
    try {
        ...
        consecutiveMisses++;
        if (consecutiveMisses >= STALE_AFTER_MISSES) {
            showBanner("Lost contact with device — retrying… (last error: " + err.message + ")");
        }
        return; // keep whatever was last on screen rather than blanking it
    }
    ...
    co2El.className = "v " + co2Class(j.co2);
